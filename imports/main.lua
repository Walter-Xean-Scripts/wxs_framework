...
local function loadModule(self, module)
    ...
    local chunk = LoadResourceFile("wxs_framework", ('imports/%s/%s.lua'):format(dir, context))
    local shared = LoadResourceFile("wxs_framework", ('imports/%s/shared.lua'):format(dir))
    ...
    if chunk then
        local fn, err = load(chunk, ('@@wxs_framework/imports/%s/%s.lua'):format(module, context))
